...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("No colon"); //throw a TreeException with a useful message
		...
		{
			if(root.getNodeWithName(parent) == null) {
		...
		parentNode.children.add(parentNode);//For each name in childrenArray, create a new node and add that node to parentNode.
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Check first parameter");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Check second parameter");
